...
- 不要用 `sessions_send(sessionKey="starter-material", ...)` 这种写法；`starter-material` 只是示例 agentId，不是 sessionKey
- 需要调用已配置角色时，优先用 `sessions_spawn`，并显式传 `agentId`
- 推荐分工：`<当前前缀>-material` 负责素材，`<当前前缀>-thinktank` 负责选题和角度，`<当前前缀>-creator` 负责大纲和初稿，`<当前前缀>-tech` 负责配置和排障
